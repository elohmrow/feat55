...
<html xml:lang="${cmsfn.language()}" lang="${cmsfn.language()}">
  <head>
    ...
    [#-- To load resources you can link them manually (e.g. line below) --]
      [#-- <link rel="stylesheet" type="text/css" href="${ctx.contextPath}/.resources/pure-module/webresources/css/bootstrap.css" media="all" /> --]
      [#-- <script src="${ctx.contextPath}/.resources/pure-module/webresources/js/jquery.js"></script> --]
    [#-- or via theme --]
    ...
    [#-- uncomment next line to use resfn templating functions to load all css which matches a globbing pattern --]
      [#-- ${resfn.css(["/pure-module/.*css"])!} --]
      ${resfn.css(["/pure-module/webresources/css/pure.css","/pure-module/webresources/css/.*css"])!}
  </head>
  <body class="pure-home ${cmsfn.language()}">

    [@cms.area name="grid"/]
    [#-- use resfn to load all js which matches the globbing pattern or link resources manually or via theme --]
    [#-- ${resfn.js(["/pure-module/.*js"])!} --]
  </body>
</html>
